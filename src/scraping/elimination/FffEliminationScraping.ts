...
export default class FffEliminationScraping {
  ...
  public async runCompetition(competitionDefault: ICompetitionDefault) {
    ...
    let pageSeason = await this.axios.get(FffConstants.URL_DEFAULT + '/' + competitionDefault.aux.url + '/calendrier_resultat');
    ...
    for (let i = 0; i < end; i++) {
      ...
      if (numberSeason >= FffConstants.START_SEASON) {
        ...
        let page = await this.axios.get(FffConstants.URL_DEFAULT + '/' + competitionDefault.aux.url + '/calendrier_resultat?sai=' + numberSeason);
